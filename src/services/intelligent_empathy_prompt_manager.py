...
class IntelligentEmpathyPromptManager:
    ...
    async def _restructure_for_anxiety(
        ...
        # 3. Add reassurance
        reassurance = "No hay presión alguna. Vamos a tu ritmo, paso a paso."
        
        # 4. Provide clear next step
        next_step = "¿Qué aspecto te gustaría entender mejor primero?"
        
        # Restructure with calming flow
        return f"{validation}\n\n{simplified}\n\n{reassurance} {next_step}"
    ...
    def _increase_warmth(self, text: str) -> str:
        ...
        if "importante" in text:
            text = text.replace("importante", "importante para ti")
        
        # Add supportive language
        if not any(phrase in text for phrase in ["estoy aquí", "juntos", "apoyo"]):
            text += " Recuerda que estoy aquí para apoyarte."
    ...
    def measure_empathy_quality(self, response: str) -> Dict[str, float]:
        ...
        # Measure warmth
        warmth_indicators = ["entiendo", "comprendo", "aprecio", "me alegra", "juntos"]
        metrics["warmth_score"] = sum(1 for indicator in warmth_indicators if indicator in response.lower()) / len(warmth_indicators)
        
        # Measure validation
        validation_indicators = ["válido", "natural", "normal", "tiene sentido", "razón"]
        metrics["validation_score"] = sum(1 for indicator in validation_indicators if indicator in response.lower()) / len(validation_indicators)
        
        # Measure personalization
        personal_pronouns = ["tu", "tú", "ti", "tuyo", "tuya"]
        metrics["personalization_score"] = min(sum(response.lower().count(pronoun) for pronoun in personal_pronouns) / 10, 1.0)
        
        # Measure clarity
        avg_sentence_length = len(response.split()) / max(len(response.split(".")), 1)
        metrics["clarity_score"] = max(0, 1 - (avg_sentence_length - 15) / 30)  # Optimal around 15 words/sentence
        
        # Measure support
        support_indicators = ["ayudar", "apoyar", "aquí para", "conmigo", "acompañar"]
        metrics["support_score"] = sum(1 for indicator in support_indicators if indicator in response.lower()) / len(support_indicators)
